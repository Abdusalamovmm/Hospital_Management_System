...
Public Class Admit
    ...
    Private Function performQueryStatement() As String
        ...
        If DateIn1Txt.Text = "" Then
            ...
            Catch ex As Exception
                ...
                MessageBox.Show("Не удается выполнить запрос!", "НЕДОПУСТИМЫЙ ЗАПРОС")
            End Try
        ElseIf Not DateIn1Txt.Text = "" And DateOut1Txt.Text = "" Then
            ...
            Try
                ...
                'ClearAdmitDetails()
                'ClearPatientInfo()
                Update_Rooms(RoomNo1Txt.Text, "Да")
                UpdateTable()
            Catch ex As Exception
                myconnection.Close()
                'MessageBox.Show(ex.Message)
    ...
    Private Sub PrintBtn_Click(sender As Object, e As EventArgs) Handles PrintBtn.Click
        ...
        If PIDTxt.Text = "" Then
            MessageBox.Show("Нет пациента, которого можно было бы распечатать")
            Exit Sub
        End If
        'validate function
        If Not Validate_Print_Data() Then
    ...
    Private Sub PrintDocument1_PrintPage(sender As Object, e As Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        ...
        e.Graphics.DrawString("Принять/выписать", ReportFont, Brushes.Chocolate, 250, 100)
        e.Graphics.DrawString("Имя - " & NameTxt.Text & "", ReportBodyFont, Brushes.Chocolate, 100, 150)
        e.Graphics.DrawString("№ Палаты - " & room_no & "", ReportBodyFont, Brushes.Chocolate, 100, 250)
        e.Graphics.DrawString("Тип палаты - " & room_type & "", ReportBodyFont, Brushes.Chocolate, 100, 300)
    End Sub
    ...
    Private Function Validate_Print_Data() As Boolean
        If RoomNoTxt.SelectedIndex = -1 And RoomNo1Txt.Text = "" Then
            MessageBox.Show("Отсутствующей № комнаты ")
            Return False
        End If
        If RoomTypeTxt.SelectedIndex = -1 And RoomType1Txt.Text = "" Then
            MessageBox.Show("Отсутствующий тип номера")
            Return False
        End If
        If Not DateIn1Txt.Visible Then
            If DateInTxt.SelectedIndex = -1 Then
                MessageBox.Show("Отсутствующие данные В")
                Return False
            End If
            If StatusTxt.Text = "Оплачено" And Not Form1.Number_Validator(FeesTxt.Text) Then
                MessageBox.Show("Missing fees")
                Return False
            End If
            If StatusTxt.Text = "Не оплачено" And Not FeesTxt.Text = "" Then
                MessageBox.Show("Невозможно отобразить сборы, если они не оплачены")
                Return False
            End If
        End If
        If Not DateOut1Txt.Visible Then
            ...
            If Not Form1.Number_Validator(FeesTxt.Text) Then
                MessageBox.Show("Неверное значение поля")
                Return False
            End If
            If StatusTxt.Text = "" Or StatusTxt.Text = "Not Paid" Then
                MessageBox.Show("Не может быть произведен платеж без уплаты сборов")
                Return False
            End If
        End If
        Return True
    ...
    Private Sub FeesTxt_TextChanged(sender As Object, e As EventArgs) Handles FeesTxt.TextChanged
